...
pub fn new(track: Vec<(PathBuf, &String)>) -> Result<(), Box<dyn Error>> {
    ...
    writer.write_event(Event::Start(BytesStart::new("playlist").with_attributes(vec![
    ...
    for (id, path) in track.iter().enumerate() {
        write_track(&mut writer, id, path_to_vlcpath(path.0.to_str().unwrap()).as_str())?;
    }

    writer.write_event(Event::End(BytesEnd::new("trackList")))?;
    writer.write_event(Event::Start(BytesStart::new("extension").with_attributes(vec![
...
fn write_vlc_item(writer: &mut Writer<File>, tid: usize) -> Result<(), Box<dyn Error>> {
    let element_start = BytesStart::new("vlc:item").with_attributes(vec![("tid", tid.to_string().as_str())]);
