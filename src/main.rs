use std::{{fs, io}, env, error::Error, path::{Path, PathBuf}, process::{Command, exit}, time::Instant};
...
async fn main() -> Result<(), Box<dyn Error>> {
	...
	loop {
		...
		if ffmpeg_check && chrome_check {
			...
			utils_check::download_and_extract_archive(ffmpeg_url, &ffmpeg_destination, &extract_path).await.expect("Erreur lors du téléchargement de FFmpeg.");
		} else if !chrome_check && ffmpeg_check {
			utils_check::download_and_extract_archive(chrome_url, &chrome_destination, &extract_path).await.expect("Erreur lors du téléchargement de Chrome.");
		} else {
			utils_check::download_and_extract_archive(chrome_url, &chrome_destination, &extract_path).await.expect("Erreur lors du téléchargement de Chrome.");
			utils_check::download_and_extract_archive(ffmpeg_url, &ffmpeg_destination, &extract_path).await.expect("Erreur lors du téléchargement de FFmpeg.");
		}
...
async fn start(url_test: &Vec<String>) -> Result<(), Box<dyn Error>> {
	...
	let episod_url = html_parser::recursive_find_url(&driver, url_test.last().unwrap(), base_url).await?;
	println!("dump url\n{:#?}", episod_url);
	...
	for (_name, url) in episod_url.clone() {
		if url.starts_with("http") {
			...
			html_parser::fetch_url(x.json()["file"].as_str().unwrap(), &_name.trim().replace(":", "")).await?;
		}
	}
	...
	let handles: Vec<_> = paths
		.filter_map(|entry| {
			...
			if file_path.is_file() {
				...
				Some(std::thread::spawn(move || web::download_build_video(&output_path.to_str().unwrap().to_string().as_str(), name)))
			} else {
				None
			}
		})
