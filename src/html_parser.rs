...
pub async fn recursive_find_url(driver: &WebDriver, _url_test: &str, base_url: &str) -> Result<HashMap<String, String>, Box<dyn Error>> {
...
pub async fn fetch_url(url: &str, file_name: &str) -> Result<(), Box<dyn Error>> {
	...
	if body.status().is_success() {
		...
		let link = &split[2];
		io::copy(&mut web::web_request(&client, link).await?.text().await?.as_bytes(), &mut out).expect("Error copy");
